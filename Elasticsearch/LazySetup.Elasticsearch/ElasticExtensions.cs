...
{
    public class ElasticExtensions : IElasticExtensions
    {
        public int MaxDepth { get; }
        private readonly IElasticClient _elasticClient;

        public ElasticExtensions(IElasticClient elasticClient, int maxDepth)
        {
            MaxDepth = maxDepth;
            _elasticClient = elasticClient;
        }

        public async Task<ElasticLinkedResponse<T>> LinkedSearchAsync<T>(Func<SearchDescriptor<T>, ISearchRequest> selector = null, CancellationToken cancellationToken = new CancellationToken()) where T : class
        {
